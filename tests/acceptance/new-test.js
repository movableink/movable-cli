...
describe('Acceptance: movable new', function() {
  ...
  it('movable new foo, where foo does not yet exist, works', co.wrap(function *() {
    yield movable([
      'new',
      'foo',
      '--skip-npm',
  ...
  it('movable new with empty app name fails with a warning', co.wrap(function *() {
    let err = yield expect(movable([
      'new',
      '',
    ])).to.be.rejected;
  ...
  it('movable new without app name fails with a warning', co.wrap(function *() {
    let err = yield expect(movable([
      'new',
    ])).to.be.rejected;
  ...
  it('movable new with app name creates new directory and has a dasherized package name', co.wrap(function *() {
    yield movable([
      'new',
      'FooApp',
      '--skip-npm',
  ...
  it('Can create new movable project in an existing empty directory', co.wrap(function *() {
    fs.mkdirsSync('bar');
  ...
  it('Cannot create new movable project in a populated directory', co.wrap(function *() {
    fs.mkdirsSync('bar');
    fs.writeFileSync(path.join('bar', 'package.json'), '{}');

    let error = yield expect(movable([
      'new',
      'foo',
      '--skip-npm',
      '--skip-bower',
  ...
  it('Running movable new inside of movable-cli project automatically skips git', co.wrap(function *() {
    yield movable([
      'new',
      'foo',
      '--skip-npm',
  ...
  it('movable new uses yarn when blueprint has yarn.lock', co.wrap(function *() {
  ...
  it('movable new without skip-git flag creates .git dir', co.wrap(function *() {
    yield movable([
      'new',
      'foo',
      '--skip-npm',
  ...
  it('movable new cleans up after itself on error', co.wrap(function *() {
  ...
  it('movable new with --dry-run does not create new directory', co.wrap(function *() {
    yield movable([
      'new',
      'foo',
      '--dry-run',
  ...
  it('movable new with --directory uses given directory name and has correct package name', co.wrap(function *() {
