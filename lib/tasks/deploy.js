...
class DeployTask extends Task {
  async run(options) {
    ...
    const remote = new GitRemote(options.environment);
    ...
    if (!hasExistingValidRemote) {
      const { auth, user, company } = await this.refreshUserInfo();
      ...
      if (existingRemoteURL) {
        ...
        this.createRepository(remote.toString());
        remote.add();
      }
    }
    ...
    try {
      ...
      ui.writeLine('deployment failed: ', e.message);
    }
  }
