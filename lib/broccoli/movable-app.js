...
class MovableApp {
  ...
  constructor(defaults, options) {
    ...
    this.nodeModulesPath = [path.resolve(this.project.root, './node_modules')];

    const monoNodeModules = path.resolve(this.project.root, '../../node_modules');
    if (existsSync(monoNodeModules)) {
      this.nodeModulesPath.push(monoNodeModules);
    }
  ...
  */
  ...
  addRollupTree(tree, rollup, opts={}) {
    rollup.input = path.relative(tree._directoryPath,
                                 this._resolveLocal(rollup.input));

    let rolledUp = new Rollup(tree, {
      nodeModulesPath: this.nodeModulesPath,
      rollup
    });

    if (opts.destDir) {
      rolledUp = new Funnel(rolledUp, {
        files: [rollup.output.file],
        srcDir: '/',
        destDir: opts.destDir
      });
    }

    this.extraTrees.push(rolledUp);
  }

  addCopyTree(tree, opts={}) {
    const input = path.relative(tree._directoryPath,
                                this._resolveLocal(opts.input));

    const srcDir = path.dirname(input);
    const file = path.basename(input);

    const output = new Funnel(tree, {
      files: [file],
      srcDir: `/${srcDir}`,
      destDir: opts.destDir
    });

    this.extraTrees.push(output);
  }
