...
module.exports = Command.extend({
  name: 'new',
  description: `Creates a new directory and runs ${chalk.green('studio init')} in it.`,
  ...
  run(commandOptions, rawArgs) {
    ...
    if (!packageName) {
      message = chalk.yellow(`The \`studio ${this.name}\` command requires a name to be specified. For more details, use \`studio help\`.`);
    ...
    if (packageName === '.') {
      ...
      message = `Trying to generate an ${blueprintName} structure in this directory? Use \`studio init\` instead.`;
